...
class GroupLinearBandit:
    ...
    def get_opt_policy(self):
        def opt_policy(state: np.ndarray, group_id: int):
            ...
            feature_mat = np.array(
                [self.feature_func(state, action_idx, group_id) for action_idx in range(self.action_num)],
                dtype=np.float32,
            )
    ...
    def evaluate_reward(self, policy):
        ...
        group_id_mat = np.floor(
            np.random.uniform(0, self.group_num, size=(self.num_trials_for_eval))
        ...
        for index in range(self.num_trials_for_eval):
            ...
            feature_mat = [self.feature_func(state, act_index, group_id) for act_index in range(self.action_num)]
...
class GroupLinearBanditSep:
    ...
    def sample(self, action, group_id) -> float:
        ...
        assert np.shape(feature) == np.shape(reward_param), "The feature is invalid."
        ...
        if feature_type == "same":
            if group_id % 2 == 0:
                for idx in range(state_dim):
                    feature[2 * idx] = (action / num_action + 1) * np.cos(
                        state[idx] * np.pi
                    )  ###maybe add group_id related qty
            ...
            if group_id % 2 == 0:
                ...
                for idx in range(state_dim):
                    feature[2 * idx + 1] = (action / num_action + 1) * np.cos(
                        state[idx] * np.pi
                    )  ###maybe add group_id related qty
