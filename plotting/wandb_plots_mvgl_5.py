...
def get_actual_iterations(method, start_iteration, num_points):
    if method in ['rdpo_with_is', 'rdpo_with_is_none']:
        return np.array(range(start_iteration, start_iteration + num_points * 100, 100))
    else:  # 'rdpo_without_is' and 'cgd'
        return np.array(range(start_iteration, start_iteration + num_points * 10, 10))
...
def plot_loss(data, deterministic_ratio, save_path, start_iteration=100):
    ...
    for method, (label, color) in methods.items():
        if method in data and deterministic_ratio in data[method]:
            ...
            # Filter data to only show first 2000 iterations
            mask = actual_iterations <= 2000
            actual_iterations = actual_iterations[mask]
            mean = mean[mask]
...
for run in runs:
    ...
    if dpo_type == 'rdpo':
        ...
        ipo_grad_type = run.config.get('ipo_grad_type', '')
        
        logging.info(f"IS enabled: {is_enabled}, IS weights: {is_weights}, IPO grad type: {ipo_grad_type}")
        
        if ipo_grad_type == 'noisy_ipo':
            method_key = 'noisy_ipo'
        elif is_enabled and is_weights == '0.333,0.333,0.333':
...
# Write data to CSV
write_data_to_csv(data, SAVE_DIR, start_iteration=200)
...
for deterministic_ratio in all_ratios:
    ...
    plot_loss(data, deterministic_ratio, save_path, start_iteration=200)
