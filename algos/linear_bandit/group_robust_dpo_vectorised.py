...
class GroupRobustDirectPolicyOptimizationVectorised:
    ...
    def batch_update_once(self, dataset: List[GroupTransition], batch_size: int, unique_group_ids: Set[int]) -> float:
        ...
        neg_cur_data_grad = self.reg_coef * coef * feature_diff_all
        group_grad[group_id] = (
            np.sum(-neg_cur_data_grad, axis=0) / cur_group_counts[group_id]
        ...
        if self.l2_reg_rdpo != 0:
            ...
            grad += (
                2 * self.l2_reg_rdpo * self.param / len(sampled_group_transitions)
            )  # theta-gradient on loss L2 norm λ . ||θ||_F
    ...
    def batch_update_closed_form(
        ...
        neg_cur_data_grad = self.reg_coef * coef * feature_diff_all
        group_grad[group_id] = (
            np.sum(-self.group_weights[group_id] * neg_cur_data_grad, axis=0) / cur_group_counts
        )  ############### had self.group_weights[group_id] scaling before
    ...
    def evaluate_weighted_ipo_loss(self, dataset: List[GroupTransition], policy=None) -> float:
        ...
        loss = np.sum(
            self.group_weights[group_id] * np.square(coef) + self.adj[group_id] / np.sqrt(self.group_counts[group_id])
        ) / len(dataset)
        loss = (
            loss * self.group_num
    ...
    def evaluate_weighted_loss(self, dataset: List[GroupTransition], policy=None) -> float:
        ...
        log_ratio_diff = self.reg_coef * feature_diff_all @ self.param.reshape(self.feature_dim, 1)
        loss = np.sum(
            -self.group_weights[group_id] * np.log(sigmoid(log_ratio_diff))
            + self.adj[group_id] / np.sqrt(self.group_counts[group_id])
        )
        loss /= len(dataset)
